...
/**
 * This is the view model for the game activity in medium mode.
 */
class MediumGameViewModel : ViewModel()
{
    ...
    fun enableClick()
    {
        if ( getViewMatrix().size < 2 || isPair ) enableClickIn( 550 )
        else if ( getViewMatrix().size == 2 && !isPair ) enableClickIn( 1800 )
    }
    /**
    ...
    {
        val cardImages = arrayOf( R.drawable.card_fire, R.drawable.card_fire, R.drawable.card_ice, R.drawable.card_strawberry,
            R.drawable.card_ice, R.drawable.card_feline, R.drawable.card_feline, R.drawable.card_strawberry )
        cardImages.shuffle()
        for ( i in 0 .. 7 ) getCards().add( Card(i, cardImages[i]) )
    }
    /**
